...
function finish(message = '') {
  ...
  $('privacy').textContent = 'Microphone off · Reports stay local. No callback arranged.';
...
function connected(call, preview = false) {
  ...
  $('privacy').textContent = preview ? 'Local microphone test · Demo report saved after hang-up.' : 'Microphone on · Azure voice · Demo report saved after hang-up.';
...
(async () => {
  try {
    ...
    if (config.preview) $('demo-note').textContent = 'Offline preview · No live recognition · Automatic demo reports';
    $('call').disabled = false;
