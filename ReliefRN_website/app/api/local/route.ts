...
async function manager(area:Area):Promise<Place[]>{if(area.state!=='VA')return [];const local=(area.locality||area.label.split(',')[0]).replace(/\s+(city|county)$/i,'').trim().toLowerCase();
 // Several people are listed per locality. Prefer the emergency-management
 // coordinator, skip entries without a street address (", VA" geocodes to the
 // middle of the state), and never show a "local" manager 30+ miles away.
 const candidates=managers.filter(x=>x.locality.replace(/\s+(city|county)$/i,'').trim().toLowerCase()===local&&/\d/.test(x.address||'')).sort((a,b)=>Number(/emergency/i.test(b.title||''))-Number(/emergency/i.test(a.title||'')));
 for(const m of candidates.slice(0,3)){const g=await geocode(m.address);if(!g[0]||miles(area,g[0])>30)continue;
  return [{id:'manager-'+m.locality+'-'+m.name,kind:'manager',name:m.name?`${m.name} · ${m.title}`:`${m.locality} Emergency Management`,address:m.address.replace(/\s+,/g,','),lat:g[0].lat,lng:g[0].lng,phone:m.phone||undefined,email:m.email||undefined,url:'https://lemd.vdem.virginia.gov/public/',source:'official'}];}
 return [];}
